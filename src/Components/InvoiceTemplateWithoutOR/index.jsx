...
const InvoiceTemplateWithoutOR = ({ NewEvent, details, onInvoiceExecuted, closeNotification }) => {
  ...
  useEffect(() => {
    if (companyInfo?.logo) {
      axios.get(`/logo-base64?url=${encodeURIComponent(companyInfo.logo)}`)
        .then(res => setLogoBase64(res.data.base64))
        .catch(() => {});
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [companyInfo]);
  ...
  JsBarcode(canvas, `FAC-${d.num}`, { format: "CODE128", displayValue: false, width: 1.4, height: 28, margin: 0 });
  ...
  const doc = {
    ...
    content: [
      ...
      {
        table: {
          widths: ["*", logoBase64 ? 70 : "auto"],
          body: [[
            {
              stack: [
                { text: "FACTURE", fontSize: 15, bold: true, color: C.white },
