...
const DevisTemplate = ({ editedEvent, details, onInvoiceExecuted, closeNotification }) => {
  const { Client, Vehicle, date, deposit } = editedEvent;
  const [user] = useAuthState(auth);
  const axios  = useAxios();
  const [companyInfo, setCompanyInfo] = useState({ name: "Garage", address: "", phone: "", email: "", codePostal: "", ville: "", dayValidityQuote: 30 });
  ...
  const d = {
    ...
    company: {
      ...
      validity: addDays(editedEvent?.createdAt || new Date().toISOString(), companyInfo?.dayValidityQuote),
    },
    vehicle: {
      ...
      /* BANDE TITRE */
      {
        table: {
          ...
          body: [[
            {
              stack: [
                { text: "DEVIS", fontSize: 15, bold: true, color: C.white },
            ...
            },
            {
              stack: [
                ...
                { text: `Valable jusqu'au ${d.company.validity}`, fontSize: 7.5, color: "#A5B4FC", alignment: "right", marginTop: 3 },
              ],
  ...
  function generatePdf() {
    pdfMake.createPdf(doc).download(`Devis_${d.num}_${new Date().toISOString().split("T")[0]}.pdf`);
