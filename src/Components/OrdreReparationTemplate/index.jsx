...
/* ── Palette commune à tous les templates ── */
const C = {
  primary:   "#4F46E5",
  primaryLight: "#EEF2FF",
  primaryMid:   "#C7D2FE",
...
/* ── Bloc info (Garage / Véhicule / Client) ── */
      ...
      [{
        stack: rows.map(({ label, value, bold: b }) =>
          label
            ? {
                columns: [
                  { text: label, fontSize: 7.5, color: C.textLight, width: 42 },
                  { text: value || "—", fontSize: 7.5, bold: !!b, color: C.textDark },
                ],
                margin: [0, 1.5, 0, 1.5],
              }
...
const OrdreReparationTemplate = ({
  ...
  const [companyInfo, setCompanyInfo] = useState({
    name: "Garage", address: "", phone: "", email: "", codePostal: "", ville: "",
  });
  ...
  const d = {
    ...
    rdvDate: date ? new Date(date).toLocaleDateString("fr-FR") : "",
    endDate: editedEvent?.endDate
      ? `${new Date(editedEvent.endDate).toLocaleDateString("fr-FR")} ${
          String(editedEvent.endHour   || 0).padStart(2, "0")}h${
          String(editedEvent.endMinute || 0).padStart(2, "0")}`
      : "",
  ...
  JsBarcode(canvas, `OR-${d.num}`, { format: "CODE128", displayValue: false, width: 1.4, height: 28, margin: 0 });
  const barcodeB64 = canvas.toDataURL("image/png");
  ...
  const doc = {
    ...
    content: [
      /* BANDE TITRE */
      {
        table: {
          ...
          body: [[
            {
              stack: [
                { text: "ORDRE DE RÉPARATION", fontSize: 15, bold: true, color: C.white },
                { text: `N° ${d.num}  ·  ${new Date().toLocaleDateString("fr-FR")}`, fontSize: 8.5, color: C.primaryMid, marginTop: 3 },
              ],
            ...
            },
            {
              stack: [
                ...
                { text: d.endDate ? `Fin : ${d.endDate}` : "", fontSize: 7.5, color: "#A5B4FC", alignment: "right", marginTop: 3 },
              ],
      ...
      /* BANDE BARCODE */
      {
        table: {
          widths: ["*"],
          body: [[{
            image: barcodeB64, fit: [180, 28], alignment: "center",
            fillColor: C.primaryLight, border: [false, false, false, false], margin: [0, 5, 0, 5],
          }]],
        },
        layout: "noBorders",
        marginBottom: 14,
      ...
      /* TABLEAU LIGNES */
      {
        table: {
          ...
          body: [
            [
              { text: "Code",       style: "th" },
              { text: "Désignation / Travaux", style: "th" },
              { text: "P.U. HT",   style: "th" },
              { text: "P.U. TTC",  style: "th" },
              { text: "Qté",       style: "th" },
              { text: "Total HT",  style: "th" },
              { text: "Total TTC", style: "th" },
              { text: "Remise",    style: "th" },
            ],
            ...d.items.map((item, i) => [
              td(item.code,     i % 2 !== 0),
              td(item.label,    i % 2 !== 0),
        ...
        columns: [
          { text: "", width: "*" },
          {
            ...
            table: {
              ...
              body: [
                [
                  { text: "Total HT",  fontSize: 8.5, color: C.textMid,  margin: [8, 5, 8, 5], border: [false, false, false, true], borderColor: ["", "", "", C.border] },
                  { text: `${d.totals.ht} €`, fontSize: 8.5, bold: true, color: C.textDark, alignment: "right", margin: [8, 5, 8, 5], border: [false, false, false, true], borderColor: ["", "", "", C.border] },
                ],
                [
                  { text: "TVA 20%",   fontSize: 8.5, color: C.textMid,  margin: [8, 5, 8, 5], border: [false, false, false, true], borderColor: ["", "", "", C.border] },
                  { text: `${d.totals.tva} €`, fontSize: 8.5, bold: true, color: C.textDark, alignment: "right", margin: [8, 5, 8, 5], border: [false, false, false, true], borderColor: ["", "", "", C.border] },
                ],
                [
                  { text: "TOTAL TTC", fontSize: 10, bold: true, color: C.white, fillColor: C.primary, margin: [8, 7, 8, 7], border: [false, false, false, false] },
                  { text: `${d.totals.ttc} €`, fontSize: 10, bold: true, color: C.white, fillColor: C.primary, alignment: "right", margin: [8, 7, 8, 7], border: [false, false, false, false] },
                ],
                [
                  { text: "Acompte",  fontSize: 8, color: C.textMid, margin: [8, 4, 8, 4], border: [false, false, false, false] },
                  { text: `${d.deposit.toFixed(2)} €`, fontSize: 8, color: C.textDark, alignment: "right", margin: [8, 4, 8, 4], border: [false, false, false, false] },
                ],
              ],
            },
            layout: {
      ...
      /* NOTE LÉGALE */
      { text: d.company.noteLegal, fontSize: 7, color: C.textLight, italics: true, marginBottom: 20 },
      ...
      /* SIGNATURES */
      {
        table: {
          ...
          body: [[
            {
              stack: [
                { text: "Signature du réceptionnaire", fontSize: 8, color: C.textMid, marginBottom: 28 },
                { canvas: [{ type: "line", x1: 0, y1: 0, x2: 175, y2: 0, lineWidth: 0.5, lineColor: C.border }] },
              ],
            ...
            {
              stack: [
                { text: "Signature du client", fontSize: 8, color: C.textMid, alignment: "right", marginBottom: 28 },
                { canvas: [{ type: "line", x1: 0, y1: 0, x2: 175, y2: 0, lineWidth: 0.5, lineColor: C.border }] },
              ],
              border: [false, false, false, false],
            },
    ...
    footer: (currentPage, pageCount) => ({
      table: {
        widths: ["*", "auto"],
        body: [[
          { text: `${d.company.name}  ·  ${d.company.address}  ·  ${d.company.phone}`, fontSize: 7, color: C.textLight, border: [false, true, false, false], borderColor: ["", C.border, "", ""], margin: [28, 5, 0, 0] },
