...
/* ─────────────────────────────────────────────────────────
   Génération SVG code-barres (JsBarcode)
───────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────
   Extraction des données utiles d'un item panier
───────────────────────────────────────────────────────── */
function itemFields(item) {
  ...
  const prixTTC   = item.prixTTC   != null ? Number(item.prixTTC).toFixed(2)   + " €" : "—";
  const prixAncien = item.prixAncien != null ? Number(item.prixAncien).toFixed(2) + " €" : null;
  const promoDebut = item.promoDebut || null;
  const promoFin   = item.promoFin   || null;
  const isPromo    = item.isPromo && !!(prixAncien && promoDebut && promoFin);
  const empl       = article.Emplacements?.[0]?.code
                  || article.Emplacement?.code
                  || article.emplacement?.code
                  || "";
  return { libelle, reference, barValue, prixTTC, prixAncien, promoDebut, promoFin, isPromo, empl };
}

/* ─────────────────────────────────────────────────────────
   BUILDER HTML — Réglette (petites étiquettes, plusieurs/page)
───────────────────────────────────────────────────────── */
function buildReglette(item, wCm, hCm) {
  const { libelle, reference, barValue, prixTTC, prixAncien, promoDebut, promoFin, isPromo, empl } = itemFields(item);
  ...
  const priceBlock = isPromo ? `
    <div style="display:flex;align-items:center;gap:4px;justify-content:center;padding:2px 0">
      <span style="font-size:11px;font-weight:700;color:#c62828;text-decoration:line-through;background:#e0e0e0;padding:1px 6px;border-radius:2px">${prixAncien}</span>
      <span style="font-size:17px;font-weight:900;background:#bdbdbd;padding:2px 8px;border-radius:2px">${prixTTC}</span>
    </div>
  ...
  ` : `
    <div style="text-align:center;padding:3px 0">
      <span style="font-size:18px;font-weight:900;background:#bdbdbd;padding:2px 12px;border-radius:2px">${prixTTC}</span>
    </div>
  `;
  ...
  return Array.from({ length: item.qty }).map(() => `
    <div style="
      ...
      border:1px dashed #aaa; padding:2px 3px; background:#f5f5f5;
      display:inline-flex; flex-direction:column; justify-content:space-between;
      margin:1.5mm; break-inside:avoid; vertical-align:top;
    ">
      <div>
...
/* ─────────────────────────────────────────────────────────
   BUILDER HTML — Page entière (A4/A5/A6 portrait ou paysage)
───────────────────────────────────────────────────────── */
function buildFullPage(item) {
  ...
  const priceBlock = isPromo ? `
    <div style="display:flex;align-items:center;gap:16px;justify-content:center;padding:18px 0">
      <span style="font-size:38px;font-weight:700;color:#c62828;text-decoration:line-through;background:#e0e0e0;padding:6px 18px;border-radius:4px">${prixAncien}</span>
      <span style="font-size:64px;font-weight:900;background:#bdbdbd;padding:10px 30px;border-radius:4px">${prixTTC}</span>
    </div>
  ...
  ` : `
    <div style="text-align:center;padding:22px 0">
      <span style="font-size:64px;font-weight:900;background:#bdbdbd;padding:10px 32px;border-radius:4px">${prixTTC}</span>
    </div>
  `;

  return Array.from({ length: item.qty }).map(() => `
    <div style="
      width:100%; min-height:calc(100vh - 20mm); box-sizing:border-box; overflow:hidden;
      border:1px dashed #bbb; padding:10mm; background:#f5f5f5;
...
/* ─────────────────────────────────────────────────────────
   Construction du HTML final d'impression
───────────────────────────────────────────────────────── */
function buildPrintHTML(panier, format, labelW, labelH) {
  ...
  let body;
  if (format === "reglette") {
    body = `<div style="display:flex;flex-wrap:wrap;align-content:flex-start">
...
<head>
  ...
  <style>
    @page { size: ${fmt.pageSize} ${fmt.orientation}; margin: 8mm; }
...
/* ─────────────────────────────────────────────────────────
   Ligne du panier avec édition promo
───────────────────────────────────────────────────────── */
function PanierRow({ item, idx, onUpdate, onRemove, onQty }) {
  const [promoOpen, setPromoOpen] = useState(false);

  const setField = (field) => (e) => {
    onUpdate(idx, { [field]: e.target.value || null });
  };
  ...
  return (
    <>
      <TableRow hover>
        {/* Désignation */}
        <TableCell sx={{ py: 0.5, pl: 2, borderBottom: promoOpen ? "none" : undefined }}>
        ...
        {/* Prix TTC — éditable */}
        <TableCell align="right" sx={{ py: 0.5, borderBottom: promoOpen ? "none" : undefined }}>
          <TextField
            size="small"
            type="number"
            value={item.prixTTC ?? ""}
            onChange={(e) => onUpdate(idx, { prixTTC: e.target.value !== "" ? parseFloat(e.target.value) : null })}
            inputProps={{ step: 0.01, min: 0, style: { textAlign: "right", fontSize: 12, width: 70 } }}
        ...
        {/* Quantité */}
        <TableCell align="center" sx={{ py: 0.5, borderBottom: promoOpen ? "none" : undefined }}>
        ...
        {/* Promo toggle + supprimer */}
        <TableCell align="right" sx={{ py: 0.5, pr: 1, borderBottom: promoOpen ? "none" : undefined }}>
          <Box display="flex" alignItems="center" justifyContent="flex-end" gap={0.5}>
            <Tooltip title={promoOpen ? "Masquer promo" : "Configurer promo"}>
              <Chip
                label="Promo"
                size="small"
                variant={item.isPromo ? "filled" : "outlined"}
                color={item.isPromo ? "warning" : "default"}
                onClick={() => setPromoOpen(v => !v)}
                deleteIcon={promoOpen ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                onDelete={() => setPromoOpen(v => !v)}
      ...
      {/* Zone promo dépliable */}
      <TableRow>
        <TableCell colSpan={4} sx={{ p: 0, border: "none" }}>
          <Collapse in={promoOpen} unmountOnExit>
            <Box
              px={2} py={1.5}
              sx={{ bgcolor: "warning.50", borderBottom: "1px solid", borderColor: "divider" }}
            >
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    onChange={(e) => onUpdate(idx, { isPromo: e.target.checked })}
                  />
                }
                label={<Typography variant="caption" fontWeight={600}>Activer la promo sur cette étiquette</Typography>}
                sx={{ mb: 1 }}
              />
              <Box display="flex" gap={1.5} flexWrap="wrap">
                <TextField
                  ...
                  size="small"
                  type="number"
                  value={item.prixAncien ?? ""}
                  onChange={(e) => onUpdate(idx, { prixAncien: e.target.value !== "" ? parseFloat(e.target.value) : null })}
                  sx={{ width: 130 }}
                  inputProps={{ step: 0.01, min: 0 }}
                  disabled={!item.isPromo}
                />
                <TextField
                  label="Début promo"
                  size="small"
                  type="date"
                  value={item.promoDebut ?? ""}
                  onChange={setField("promoDebut")}
...
export default function EtiquetageModal({ open, onClose }) {
  ...
  async function fetchPricing(articleId) {
    try {
      ...
      // endpoint retourne un objet unique (findOne)
      if (data && typeof data === "object" && !Array.isArray(data) && data.prixTTC != null) {
        return data;
      }
      // fallback si tableau (ancienne version)
  ...
  async function addToPanier(article) {
    ...
    const pricing = await fetchPricing(article.id);
    setPanier(prev => {
  ...
  /* ── Mise à jour d'un champ d'un item ── */
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
        ...
        <DialogContent sx={{ p: 0 }}>
          <Box display="flex" minHeight={460}>
            ...
            {/* ─── Panier (gauche) ─────────────────────── */}
            <Box flex={1} borderRight="1px solid" sx={{ borderColor: "divider" }}
              ...
              <Box flex={1} sx={{ overflowY: "auto" }}>
                {panier.length === 0 ? (
                  <Box display="flex" alignItems="center" justifyContent="center"
                    height="100%" p={4} textAlign="center">
