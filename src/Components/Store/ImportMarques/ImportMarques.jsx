  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const rows  = raw
          .slice(1)
          .map((row, idx) => ({ ligne: idx + 2, nom: String(row[0] ?? "").trim() }))
          .filter((r) => r.nom);
        resolve(rows);
      } catch (err) {
        reject(err);
  ...
  return (
    <Box sx={{ maxHeight: 280, overflowY: "auto", border: "1px solid", borderColor: "divider", borderRadius: 1 }}>
      <Table size="small" stickyHeader>
        <TableHead>
          <TableRow>
            <TableCell sx={{ fontWeight: 700, width: 70 }}>Ligne</TableCell>
            <TableCell sx={{ fontWeight: 700 }}>Nom Marque</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {rows.map((row) => (
            <TableRow key={row.ligne}>
              <TableCell>
                <Typography variant="caption" color="text.secondary">{row.ligne}</Typography>
              </TableCell>
  ...
  return (
    <Box display="flex" flexDirection="column" gap={2}>
      <Box display="flex" gap={2} flexWrap="wrap">
        <Chip icon={<CheckCircleOutlineIcon />} label={`${report.created} créée(s)`}  color="success" variant="outlined" />
        <Chip label={`${report.skipped} ignorée(s) (doublon)`} color="default" variant="outlined" />
...
/* ─────────────────────────────────────────────────────────
   Composant principal
───────────────────────────────────────────────────────── */
export default function ImportMarques({ onSuccess }) {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const res = await axios.post("/stock/import/marques", { rows: parsedRows });
      if (res?.data?.report) {
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
        <DialogTitle>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography variant="h6">Import Excel — Marques</Typography>
              ...
              <>
                ...
                >
                  ...
                  <Typography variant="body1" gutterBottom>
                    {fileName
                      ? <><strong>{fileName}</strong> — {parsedRows?.length} marque(s) détectée(s)</>
                      : "Glissez-déposez un fichier .xlsx ou cliquez pour sélectionner"}
                  </Typography>
                  {!fileName && (
                    <Typography variant="caption" color="text.secondary">
                      Colonne A : Nom de la marque
                    </Typography>
                  )}
                </Box>
