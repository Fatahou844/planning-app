...
const CATEGORIES   = ["Grossiste","Fabricant","Distributeur","Importateur","Prestataire","Autre"];

/* ── export CSV ──────────────────────────────────────────────────────── */
function exportCSV(rows) {
  const headers = ["ID","Nom","Code","Catégorie","Statut","Ville","Région","Téléphone","Email","Remise %","Délai livraison","Franco de port"];
  const lines = [
    headers.join(";"),
    ...rows.map(f => [
      f.id,
...
export default function FournisseursModal({ open, onClose }) {
  ...
  useEffect(() => {
    if (open) { load(); setSearch(""); setFilterStatut(""); setFilterCat(""); setFilterRegion(""); }
  ...
  }, [fournisseurs, search, filterStatut, filterCat, filterRegion]);
  ...
  const handleFicheClose = () => {
    setFicheOpen(false);
    load(); // rafraîchit la liste si des modifs ont eu lieu
  };
        ...
        >
          <Typography variant="subtitle1" fontWeight={700} flex={1}>
            ...
            <Typography component="span" variant="caption" color="text.secondary" ml={1.5}>
              {loading ? "…" : `${filtered.length} / ${fournisseurs.length}`}
            </Typography>
          </Typography>

          <Tooltip title="Exporter CSV">
            <IconButton size="small" onClick={() => exportCSV(filtered)} disabled={!filtered.length}>
        ...
        <DialogContent sx={{ display: "flex", flexDirection: "column", p: 0, overflow: "hidden" }}>
          ...
          <Box sx={{ flex: 1, overflow: "auto" }}>
            {loading ? (
              ...
              <Box sx={{ textAlign: "center", py: 8 }}>
                <Typography color="text.disabled">Aucun fournisseur trouvé</Typography>
              </Box>
            ) : (
              <Table size="small" stickyHeader>
                <TableHead>
                  <TableRow>
                    {[
                      "Raison sociale", "Code", "Catégorie", "Statut",
                      "Ville / Région", "Téléphone", "Email",
                      "Remise", "Délai livr.", "Franco port", "",
                    ].map(h => (
                      <TableCell
                        key={h}
                        sx={{
                          fontWeight: 700, fontSize: 11, color: "text.secondary",
                          py: 0.9, whiteSpace: "nowrap",
                ...
                <TableBody>
                  {filtered.map(f => (
                    ...
                    >
                      ...
                      <TableCell>
                        {f.categorie
                          ? <Chip label={f.categorie} size="small" variant="outlined" sx={{ fontSize: 10 }} />
                      ...
                      <TableCell sx={{ whiteSpace: "nowrap" }}>
                        {f.telephone ? (
                          <Box
                            ...
                            component="a" href={`tel:${f.telephone}`}
                            sx={{ color: "text.primary", textDecoration: "none", fontSize: 12 }}
                          >
                      ...
                      <TableCell sx={{ whiteSpace: "nowrap", maxWidth: 180 }}>
                        {f.email ? (
                          <Box
                            display="flex" alignItems="center" gap={0.5}
                            onClick={e => e.stopPropagation()}
                            component="a" href={`mailto:${f.email}`}
                            sx={{ color: "text.primary", textDecoration: "none", fontSize: 12 }}
                          >
                            <EmailIcon sx={{ fontSize: 13, color: "text.secondary" }} />
                            <Typography variant="caption" noWrap sx={{ maxWidth: 150 }}>{f.email}</Typography>
                          </Box>
                        ) : <Typography variant="caption" color="text.disabled">—</Typography>}
                      </TableCell>
                      ...
                      {/* Délai livraison */}
                      <TableCell sx={{ fontSize: 12 }}>
                        {f.delaiLivraison != null ? `${f.delaiLivraison} j` : "—"}
                      </TableCell>

                      {/* Franco de port */}
                      <TableCell sx={{ fontSize: 12, whiteSpace: "nowrap" }}>
                        {f.francoPort != null ? `${parseFloat(f.francoPort).toFixed(0)} €` : "—"}
                      </TableCell>

                      {/* Actions */}
                      <TableCell sx={{ px: 0.5 }} onClick={e => e.stopPropagation()}>
                        <Tooltip title="Ouvrir la fiche">
                          <IconButton size="small" onClick={() => handleOpenFiche(f)}>
          ...
          >
            <Typography variant="caption" color="text.secondary">
              ...
              {filtered.length !== fournisseurs.length && ` sur ${fournisseurs.length}`}
            </Typography>
            <Box flex={1} />
            <Box display="flex" gap={1}>
              {[
                { label: "Actifs",    color: "success", count: fournisseurs.filter(f => (f.statut||"ACTIF") === "ACTIF").length },
                { label: "Inactifs",  color: "default", count: fournisseurs.filter(f => f.statut === "INACTIF").length },
                { label: "Suspendus", color: "error",   count: fournisseurs.filter(f => f.statut === "SUSPENDU").length },
