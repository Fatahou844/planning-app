...
const PreviewOR = ({ orData }) => {
  const theme = useTheme();
  const { Client, Vehicle, Details = [] } = orData || {};
    ...
    >
      ...
      >
        <Typography variant="h6" fontWeight="bold">
          Ordre de Réparation #{orData?.id}
        </Typography>
        <Typography variant="body2">
          {new Date(orData?.date).toLocaleDateString()}
        </Typography>
      </Box>
      ...
      >
        ...
        <Typography variant="body1">
          ...
          {orData?.deposit ? parseFloat(orData.deposit).toFixed(2) : "0.00"} €
        </Typography>
      </Box>
      ...
      >
        {orData?.notes || "Aucune note disponible."}
      </Typography>
    </Box>
  );
};

export default PreviewOR;
